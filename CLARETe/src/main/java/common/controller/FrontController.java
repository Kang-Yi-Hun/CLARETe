...
@WebServlet(
		...
		urlPatterns = {"*.cl"},
		initParams = { 
				@WebInitParam(name = "propertyConfig", value = "C:/Users/user/git/CLARETe/CLARETe/src/main/webapp/WEB-INF/Command.properties", description = "*.cl 에 대한 클래스의 매핑파일")}) 
...
public class FrontController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		/*
	        웹브라우저 주소창에서  *.cl 을 하면 FrontController 서블릿이 응대를 해오는데 
		...
		*/   
		...
		try {
			...
			pr.load(fis);	// pr은 읽어온 파일(Command.properties)의 내용에서 = 을 기준으로 왼쪽은 key로 보고, 오른쪽은 value 로 인식한다.
			/*
	        ...
	        */
			...
			while (en.hasMoreElements()) {
				...
				String className = pr.getProperty(key);		// value
				...
				if (className != null) {
					...
					Constructor <?> constrt = cls.getDeclaredConstructor(null);
					// 생성자 만들기
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//  웹브라우저의 주소 입력창에서 
		//  http://localhost:9090/MyMVC/member/idDuplicateCheck.cl?userid=leess 와 같이 입력되었더라면
		//String url = request.getRequestURL().toString();
		//System.out.println("~~~ 확인용 url => " + url);
		// ~~~ 확인용 url => http://localhost:9090/MyMVC/member/idDuplicateCheck.cl
		
		//  웹브라우저의 주소 입력창에서 
		//  http://localhost:9090/MyMVC/member/idDuplicateCheck.cl?userid=leess 와 같이 입력되었더라면
		String uri = request.getRequestURI().toString();
		//System.out.println("~~~ 확인용 uri => " + uri);
		// ~~~ 확인용 uri => /MyMVC/member/idDuplicateCheck.cl
